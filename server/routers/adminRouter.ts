...
export const adminCountryRouter = router({
  ...
  getMonthlyRevenue: adminProcedure
    ...
    .query(async ({ input }) => {
      ...
      const result = await db.execute(sql`
        ...
        ORDER BY month ASC
      `);
      const data = result.rows as unknown as Array<{ month: string; country: string; cnt: number; revenue: number }>;
      return { data: data ?? [] };
    }),
  ...
  getRevenueSummary: adminProcedure.query(async () => {
    ...
    `);
    return { data: result.rows as unknown as Array<{ country: string; cnt: number; revenue: number; monthRevenue: number }> };
  }),
});
