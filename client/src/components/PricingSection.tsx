...
export default function PricingSection() {
  ...
  const comingSoonMsg = isKo ? "서비스 준비 중입니다. 곧 오픈합니다!" : isJa ? "まもなくオープンします！" : "Coming soon!";
  ...
  const cards = [
    {
      tier: "Silver",
      tierLabel: getLang("실버 카드", "シルバーカード", "Silver Card"),
      planName: getLang("전자 인증 + 1년 보관", "電子認証 + 1年保管", "Certification + 1yr Storage"),
      ...
      material: getLang("실버 컬러", "シルバーカラー", "Silver Color"),
      popular: false,
      icon: CreditCard,
      features: isKo
    ...
    {
      tier: "Gold",
      tierLabel: getLang("골드 카드", "ゴールドカード", "Gold Card"),
      planName: getLang("전자 인증 + 3년 보관", "電子認証 + 3年保管", "Certification + 3yr Storage"),
      ...
      material: getLang("골드 컬러", "ゴールドカラー", "Gold Color"),
      popular: true,
      icon: Star,
      features: isKo
    ...
    {
      tier: "Platinum",
      tierLabel: getLang("플래티넘 카드", "プラチナカード", "Platinum Card"),
      planName: getLang("전자 인증 + 5년 보관 + 자필·영상", "電子認証 + 5年保管 + 自筆・映像", "Certification + 5yr + Handwritten & Video"),
      ...
      material: getLang("플래티넘 컬러", "プラチナカラー", "Platinum Color"),
      popular: false,
      icon: Sparkles,
      features: isKo
    ...
    {
      tier: "VIP",
      tierLabel: getLang("VIP 프리미엄", "VIPプレミアム", "VIP Premium"),
      planName: getLang("영구 보관 + 전체 기능 + VIP", "永久保管 + 全機能 + VIP", "Lifetime Storage + All Features + VIP"),
      ...
      material: getLang("티타늄 · 플래티넘", "チタン・プラチナ", "Titanium · Platinum"),
      popular: false,
      icon: Sparkles,
      features: isKo
      ...
      <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        >
          <div className="inline-flex items-center gap-2 bg-[#C9A961]/20 border border-[#C9A961]/30 rounded-full px-4 py-1.5 mb-5">
            ...
            <span className="text-sm text-[#C9A961] font-medium">
              {getLang("멤버십 인증 카드 & 가격 정책", "会員認証カード & 料金プラン", "Membership Card & Pricing")}
            </span>
          </div>
          <h2
          ...
          >
            {getLang("투명한 가격 정책", "透明な料金プラン", "Transparent Pricing")}
          </h2>
          ...
          <div className="inline-flex items-center gap-2 bg-red-500/20 border border-red-400/30 rounded-full px-5 py-2">
            ...
            <span className="text-red-300 text-sm font-bold">
              {getLang("2026 출시 기념 한정 특가 — 정식 출시 후 가격 인상 예정", "2026年ローンチ記念限定特価 — 正式ローンチ後に値上げ予定", "2026 Launch Special — Price increases after official launch")}
            </span>
          </div>
        </motion.div>
          ...
          {CARD_FEATURES.map((feat, i) => (
            <div key={i} className="bg-white/5 border border-white/10 rounded-xl p-4 text-center hover:bg-white/10 transition-colors">
              ...
              <p className="text-white font-semibold text-sm mb-1">
                {isKo ? feat.labelKo : isJa ? (feat as any).labelJa : feat.labelEn}
              </p>
              <p className="text-white/50 text-xs leading-relaxed">
                {isKo ? feat.descKo : isJa ? (feat as any).descJa : feat.descEn}
              </p>
            </div>
          ))}
        </motion.div>
          ...
          {cards.map((card, i) => (
            ...
            >
              ...
              <div className="mb-4">
                ...
                <span className="text-white/40 text-sm ml-1">{getLang("/ 1회", "/ 1回", "/ once")}</span>
              </div>
              ...
              {/* 신청 버튼 */}
              <button
                onClick={() => toast.info(getLang("카드 주문 기능은 곧 오픈됩니다!", "カード注文機能はまもなくオープンします！", "Card ordering coming soon!"))}
              ...
              >
                {getLang("지금 신청하기", "今すぐ申し込む", "Apply Now")}
              </button>
            </motion.div>
          ))}
        </div>
        ...
        >
          <div className="flex items-center gap-4">
            ...
            <div>
              <h3 className="text-white font-bold text-lg mb-1">
                {getLang("무료로 시작하기", "無料で始める", "Start for Free")}
              </h3>
          ...
          <div className="flex flex-col sm:flex-row items-center gap-4">
            ...
            >
              {getLang("무료로 시작하기", "無料で始める", "Start Free")}
