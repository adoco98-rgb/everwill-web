...
## 회원번호 자동 생성 시스템
- [x] DB: users 테이블에 memberCode 필드 추가 (unique, nullable)
- [x] pnpm db:push 실행
- [x] 회원번호 생성 함수: EV-[국가코드2자리]-[YYMMDD]-[전체순번 padStart(5,'0')]
- [x] 이메일 OTP 가입 완료 시 memberCode 자동 생성 저장
- [x] 휴대폰 OTP 가입 완료 시 memberCode 자동 생성 저장
- [x] 소셜 로그인(Google/Kakao/Naver/LINE) 최초 가입 시 memberCode 자동 생성 저장
- [x] 대시보드 홈: 회원번호 카드 표시
- [x] 프로필 페이지: 회원번호 표시
