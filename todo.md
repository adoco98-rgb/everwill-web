...
## 개인 인증 페이지 개선 (2024-07-08)
- [x] 신분증 업로드 후 미리보기(이미지 프리뷰) 표시
- [x] AI 스캔 인식 → 이름/생년월일/주민번호/주소 자동 추출 + 신뢰도 표시
- [x] 본인 사진 여권사진 수준 검증 가이드라인 UI 추가
- [x] 본인 사진 업로드 후 미리보기 + 신분증 대비 얼굴 일치율 검증
- [x] 업로드 완료 시 배지 표시 (업로드 완료 / 본인 확인됨)
- [x] 파일 삭제(재업로드) 버튼 추가

## 자산 등록 (Step2) 문서 업로드 우선 UX 개선 (2024-07-08)
- [x] 서류 업로드 → AI 자동 인식 → 정보 자동 입력 흐름으로 변경
- [x] 서류 유형 선택 UI (잔액증명서, 등기부등본, 보험증권, 자동차등록증 등)
- [x] 업로드 후 미리보기 표시 + AI 인식 중 로딩 상태
- [x] 인식 결과 자동 채움 → 사용자 확인/수정/저장 폼
- [x] 직접 입력도 보조 옵션으로 유지
- [x] 고령 사용자 친화적: 큰 글씨, 명확한 안내, 단계별 설명

## 새로고침 후 데이터 유지 (2024-07-08)
- [x] WillWizardPage 진행 상태 localStorage + DB 기반 자동 복원
- [x] Step4Will 저장된 유언장 자동 불러오기 (새로고침 후 복원)
- [x] 유언장 제목 회원 이름 기반 자동 입력
- [x] 유언집행자 상속 지분 1위 자동 반영
- [x] 전자서명 패드 2개 (유언자 서명 + 확인 서명)

## 5단계 완료 후 간편 유언장 작성본 + 결제 인증 안내 박스
- [x] WillWizardPage 결제 링크 /dashboard/payment → /dashboard/payments 수정
- [x] 5단계 완료 시 간편 유언장 요약 카드 자동 표시 (유언자, 상속자, 자산, 유언집행자 요약)
- [x] 결제 인증 안내 박스 개선 (법적 효력 설명 + 절차 안내 + 결제 버튼)

## 결제 페이지 단일 상품화 + 결제 수단 안내 + 노인복지후원
- [x] PaymentsPage 4개 멤버십 카드 제거 → ₩168,000 단일 상품 (모든 서비스 포함)
- [x] 단일 상품 설명 박스: 전자인증, 블록체인, 영상유언, 자필스캔, 영구보관 등 12개 서비스 총망라
- [x] 결제 수단 안내: 카카오페이, 신용카드, 계좌이체, Google Pay, Apple Pay
- [x] 노인복지후원 결제창 추가 (빈곤·독거·질환 등 노인복지 사회후원금 안내 + AI 이미지 + 금액 선택)
- [x] WillWizardPage 결제 안내 박스 "실버 멤버십 기준" → "모든 서비스 포함"으로 변경

## 간편 유언서 미리보기 + 출력 기능
- [x] 간편 유언장 작성본 카드에 "미리보기" 버튼 추가
- [x] 미리보기 모달: 유언장 전문 + 업로드 서류 포함 간편 유언서 PDF 표시
- [x] 인쇄(출력) + PDF 다운로드 기능 구현

## 사이드바 순서 변경 + 건강증명서 추가
- [x] 사이드바 "결제하기" → "전자인증·결제하기"로 변경
- [x] 사이드바 순서 재배치: 유언작성→자산등록→상속자등록→전자인증·결제하기 | 개인인증→공증서류→서명인증→영상유언→자필유언장→유언진행현황→유언인증서발급→유류분배제→유류분영상증언
- [x] 공증서류 페이지에 건강증명서 섹션 추가 (업로드 기능)
- [x] 건강증명서 발급 안내 (발급처, 절차, 비용 등)

- [x] 기본유언장 확인 페이지 (WillPreviewPage) - 무료 회원 유언장 확인+출력 기능
- [x] App.tsx에 /dashboard/will-preview 라우트 등록
