...
- [ ] 전자 유언장 법적 효력 한계 약관 명시 (현행법상 전자 유언 불인정 고지)
- [ ] 유류분 계산 "참고용" 명시 + 툴팁 (특별수익·기여분 반영 안내)
- [ ] 유언 검인 신청 절차 안내 (가정법원) 페이지/섹션 추가
- [ ] 미국 주(State)별 유언 요건 차이 고지 UI
- [ ] 일본 2025년 공정증서 디지털화 개정 내용 반영

### UX/기술 개선 (김태준 개발자 페르소나)
- [ ] 404 에러 페이지 처리 확인 및 개선
- [ ] 각 페이지별 meta title/description 설정 (SEO)
- [ ] 개인정보 암호화 안내 문구 (자물쇠 아이콘 + 한 줄 설명)
- [ ] 고객센터/1:1 문의 접근성 개선 (항상 보이는 위치)

### 디자인 개선 (박서연 디자이너 페르소나)
- [ ] 모바일 소셜 로그인 버튼 터치 영역 44px 이상 확인
- [ ] 모바일 폰트 크기 최소 16px 이상 확인 (시니어 대상)
